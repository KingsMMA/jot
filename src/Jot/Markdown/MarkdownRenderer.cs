...
public static class MarkdownRenderer
{
    ...
    public static string Shell(MarkdownPalette palette) =>
        $"<!doctype html><html><head><meta charset=\"utf-8\">" +
        $"<style>{Css(palette)}</style></head>" +
        $"<body><article id=\"content\" class=\"markdown-body\"></article></body></html>";

    public static string Css(MarkdownPalette p) => $$"""
        body {
            margin: 0;
        ...
        h1 { font-size: 2em; padding-bottom: .3em; border-bottom: 1px solid {{p.Border}}; }
        h2 { font-size: 1.5em; padding-bottom: .3em; border-bottom: 1px solid {{p.Border}}; }
        h3 { font-size: 1.25em; }
        h4 { font-size: 1em; }
        h5 { font-size: .875em; }
        h6 { font-size: .85em; color: {{p.Muted}}; }
        ...
        blockquote {
            padding: 0 1em;
            color: {{p.Muted}};
            border-left: .25em solid {{p.Border}};
        }
        table { border-collapse: collapse; display: block; width: max-content; max-width: 100%; overflow: auto; }
        table th, table td { padding: 6px 13px; border: 1px solid {{p.Border}}; }
        table tr { background: {{p.Background}}; border-top: 1px solid {{p.Border}}; }
        table tr:nth-child(2n) { background: {{p.CodeBackground}}; }
        img { max-width: 100%; }
        hr { height: .25em; padding: 0; margin: 24px 0; background: {{p.Border}}; border: 0; }
